...
# Form implementation generated from reading ui file 'SignalViewer_NewLook1.ui'
#
...
def table_creation(self,pdffilename):
    ...
    for signal_data in sig_data:
        ...
        amp=signal_data[1]
        tim=signal_data[0]
        signal_info=["{}".format(chn[i]),round(amp.mean(),5),round(amp.std(),5),round(amp.max(),5),round(amp.min(),5),round((tim.max() - tim.min()),5)]
...
class MplCanvas(FigureCanvasQTAgg):
    ...
    def __init__(self, parent=None, width=4, height=4, dpi=100):
        ...
        self.axes.set(xlabel='time (sec)', ylabel='frequency (Hz)', )
        #self.axes.grid()
...
class Canvas(FigureCanvas):
    def __init__(self, path, cmap, min , max ):
        ...
        self.ax.specgram(amplitude, NFFT=1024, Fs=fs, noverlap=900, cmap=cmap , vmin=min ,vmax=max)

        self.ax.set(xlabel='time (sec)', ylabel='frequency (Hz)', )
        self.ax.grid()
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.comboBox.currentIndexChanged.connect(lambda: self.rename_channel(self.comboBox.currentIndex()))
        ...
        self.pushButton_zoomOut.clicked.connect(lambda: self.zoom_out(self.zoomOut_scale))
        ...
        self.comboBox_color.currentIndexChanged.connect(lambda: self.Change_spectrogram_clr(self.comboBox_color.currentIndex()))
        ...
        self.comboBox_spectro.currentIndexChanged.connect(lambda: self.open_spectrogram(self.comboBox_spectro.currentIndex()))
    ...
    def rename_channel(self , channelidx):
        _translate = QtCore.QCoreApplication.translate
        if channelidx == 0:
            name1=self.Channel_1.setText(_translate("MainWindow",  self.textbox.text()))
            name1
        elif channelidx ==1:
    ...
    def Open_file(self):
        ...
        if self.channel1 == False:
            ...
            self.graphicsView_2.plotItem.vb.setLimits(xMin=min(time2), xMax=max(time2), yMin=min(data2) - 0.5,yMax=max(data2) + 0.5)
            self.x_range2 = self.graphicsView_2.getViewBox().state['viewRange'][0]
            ...
            self.graphicsView_3.plotItem.vb.setLimits(xMin=min(time3), xMax=max(time3), yMin=min(data3) - 0.5,yMax=max(data3) + 0.5)
            self.x_range3 = self.graphicsView_3.getViewBox().state['viewRange'][0]
            self.graphicsView_3.show()
            self.channel3=True
